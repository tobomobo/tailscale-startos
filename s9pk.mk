...
define SUMMARY
	...
	printf "\n"; \
	printf "\033[1;37m%s\033[0m   \033[36mv%s\033[0m\n" "$$title" "$$version"; \
	printf "%s\n" "-------------------------------"; \
...
$(BASE_NAME)_%.s9pk: $(INGREDIENTS) .git/HEAD .git/index
	@$(MAKE) --no-print-directory ingredients
	@echo "   Packing '$@'..."
	start-cli s9pk pack --icon icon.svg --arch=$* -o $@
...
publish: | all
	...
	if [ -z "$$S3BASE" ]; then \
		echo "Error: You must define \"s9pk-s3base: https://s9pks.my-s3-bucket.tld\" in ~/.startos/config.yaml"; \
		exit 1; \
	fi; \
